...
def setup_test_files():
    ...
    if not os.path.exists("doc1.txt"):
        with open("doc1.txt", "w") as f:
            f.write("Ceci est le document un, pour tester AICompress.")
    if not os.path.exists("mon_dossier_test"):
        os.makedirs("mon_dossier_test/sous_dossier_test", exist_ok=True)
        with open("mon_dossier_test/fichier_interne.txt", "w") as f:
            f.write("Fichier interne dans mon_dossier_test.")
        with open("mon_dossier_test/sous_dossier_test/profond.txt", "w") as f:
            f.write("Fichier profond dans un sous-dossier.")
    ...
    if not os.path.isdir("ai_data_samples"):
        ...
        print(
            "Veuillez exécuter le script setup_etape3_ia.sh ou le créer manuellement."
        )
    else:
        ...
        # python_code_1.py
        if os.path.exists("ai_data_samples/python_code_1.py") and not os.path.exists(
            "python_code_1.py"
        ):
            shutil.copy("ai_data_samples/python_code_1.py", "python_code_1.py")
        elif not os.path.exists("python_code_1.py"):
            with open("python_code_1.py", "w") as f:
                f.write("def sample_func():\n  pass # Fichier python_code_1.py factice")

        # english_text_1.txt
        if os.path.exists("ai_data_samples/english_text_1.txt") and not os.path.exists(
            "english_text_1.txt"
        ):
            shutil.copy("ai_data_samples/english_text_1.txt", "english_text_1.txt")
        elif not os.path.exists("english_text_1.txt"):
            with open("english_text_1.txt", "w") as f:
                f.write(
                    "This is a sample English text for testing. # Fichier english_text_1.txt factice"
                )
...
def main():
    ...
    print(
        f"État de l'analyseur IA : {'Disponible' if AI_ANALYZER_AVAILABLE else 'Non disponible'}"
    )
    print(
        f"État du support RAR : {'Disponible' if RARFILE_AVAILABLE else 'Non disponible'}\n"
    )
    ...
    for path_item in FICHIERS_POUR_AIC_TEST:
        if os.path.exists(path_item):
            ...
            print(
                f"Avertissement (Compression AIC): '{path_item}' non trouvé, sera ignoré."
            )
    ...
    if fichiers_sources_valides_aic:
        if compress_to_aic(fichiers_sources_valides_aic, NOM_ARCHIVE_AIC):
            ...
            if os.path.exists(NOM_ARCHIVE_AIC):
                print(
                    f"Taille de l'archive AIC : {os.path.getsize(NOM_ARCHIVE_AIC)} octets."
                )
        else:
    ...
    if os.path.exists(NOM_ARCHIVE_AIC):
        ...
        if extract_archive(NOM_ARCHIVE_AIC, DOSSIER_EXTRACTION_AIC):
            print(
                f"Décompression AIC réussie. Vérifiez le contenu de '{DOSSIER_EXTRACTION_AIC}'."
            )
            print(
                "IMPORTANT : Examinez les métadonnées affichées ci-dessus pour voir les résultats de l'analyse IA !"
            )
        else:
    ...
    if os.path.exists(NOM_ARCHIVE_ZIP_STD):
        if extract_archive(NOM_ARCHIVE_ZIP_STD, DOSSIER_EXTRACTION_ZIP_STD):
            print(
                f"Décompression ZIP réussie. Vérifiez le contenu de '{DOSSIER_EXTRACTION_ZIP_STD}'."
            )
        else:
            print(f"Échec de l'extraction de '{NOM_ARCHIVE_ZIP_STD}'.")
    else:
        print(
            f"Fichier '{NOM_ARCHIVE_ZIP_STD}' non trouvé. Créez-le manuellement pour ce test (ex: avec 7-Zip ou autre)."
        )
    ...
    if RARFILE_AVAILABLE:
        if os.path.exists(NOM_ARCHIVE_RAR):
            if extract_archive(NOM_ARCHIVE_RAR, DOSSIER_EXTRACTION_RAR):
                print(
                    f"Décompression RAR réussie. Vérifiez le contenu de '{DOSSIER_EXTRACTION_RAR}'."
                )
            else:
                print(f"Échec de l'extraction de '{NOM_ARCHIVE_RAR}'.")
        else:
            print(
                f"Fichier '{NOM_ARCHIVE_RAR}' non trouvé. Créez-le manuellement pour ce test (ex: avec WinRAR ou autre)."
            )
    else:
        print(
            f"Test de décompression RAR sauté car la bibliothèque 'rarfile' ou l'outil 'unrar' n'est pas disponible/configuré."
        )
