...
if physical_devices:
    print(f"GPU(s) détectée(s): {physical_devices}"); tf.config.experimental.set_memory_growth(physical_devices[0], True)
else: print("Aucun GPU détecté. L'entraînement sera sur CPU et peut être TRES long.")
...
x_train = x_train_gray.astype('float32') / 255.
x_test = x_test_gray.astype('float32') / 255.
...
encoded_volume = MaxPooling2D((2, 2), padding='same', name="encoded_latent_volume")(x) # 4x4x16
...
x_dec = Conv2D(16, (3, 3), padding='same', name="dec_conv0")(encoded_volume) # Couche initiale pour correspondre à la sortie de l'encodeur avant UpSampling
...
decoded_output = Conv2D(1, (3, 3), activation='sigmoid', padding='same', name="decoder_output_conv")(x_dec) # 32x32x1

autoencoder_cifar_gray = Model(input_img, decoded_output, name="autoencoder_cifar10_gray_v4_3b")
...
EPOCHS = 100  # Augmenter encore, la BN peut aider à entraîner plus longtemps sans diverger.
...
history = None
try:
    history = autoencoder_cifar_gray.fit(x_train, x_train,
                                epochs=EPOCHS,
    ...
    encoder_cifar10_gray = Model(input_img, encoded_volume, name="encoder_cifar10_gray_v4_3b")
    
    decoder_input_standalone = Input(shape=latent_shape, name="decoder_input_cifar10_gray_v4_3b")
    ...
    decoder_cifar10_gray = Model(decoder_input_standalone, decoder_output_standalone, name="decoder_cifar10_gray_v4_3b")
    ...
    print(f"\nSauvegarde encodeur: {ENCODER_SAVE_PATH}"); encoder_cifar10_gray.save(ENCODER_SAVE_PATH)
    print(f"Sauvegarde décodeur: {DECODER_SAVE_PATH}"); decoder_cifar10_gray.save(DECODER_SAVE_PATH)
    print("Modèles V4.3b sauvegardés.")

except Exception as e:
    print(f"Erreur entraînement/sauvegarde modèles V4.3b: {e}"); import traceback; print(traceback.format_exc())

# 5. Évaluer et Visualiser les résultats (si entraînement OK)
if history and 'decoder_cifar10_gray' in locals() and 'encoder_cifar10_gray' in locals():
    ...
    for i in range(n):
        ax = plt.subplot(2, n, i + 1); plt.imshow(x_test[i].reshape(32, 32), cmap='gray')
        ax.set_title("Originale", fontsize=8); ax.get_xaxis().set_visible(False); ax.get_yaxis().set_visible(False)
        ax = plt.subplot(2, n, i + 1 + n); plt.imshow(decoded_imgs[i].reshape(32, 32), cmap='gray')
        ax.set_title("Reconstruite", fontsize=8); ax.get_xaxis().set_visible(False); ax.get_yaxis().set_visible(False)
    plt.suptitle("CIFAR-10 Gris (V4.3b + BN): Originales vs. Reconstruites")
    plt.tight_layout(rect=[0,0,1,0.95]); plt.savefig("cifar10_gray_reconstructions_v4_3b.png")
    print("Image reconstructions sauvegardée: cifar10_gray_reconstructions_v4_3b.png"); plt.show()
    plt.figure(); plt.plot(history.history['loss'], label='Perte (train)'); plt.plot(history.history['val_loss'], label='Perte (val)')
    plt.title('Courbe Perte (CIFAR-10 Gris Autoencodeur V4.3b + BN)'); plt.ylabel('Perte'); plt.xlabel('Époque'); plt.legend(loc='upper right')
    plt.savefig("cifar10_gray_loss_curve_v4_3b.png"); print("Courbe perte sauvegardée: cifar10_gray_loss_curve_v4_3b.png"); plt.show()
