...
def analyze_file_content(file_path, log_callback=_default_log_analyzer):
    ...
    if PYTHON_MAGIC_AVAILABLE:
        try:
            ...
            if mime_type: 
                ...
                elif mime_type.startswith('image/tiff'): file_type_by_magic = "tiff_image"
                elif mime_type.startswith('image/bmp'): file_type_by_magic = "bmp_image"
                ...
                else: file_type_by_magic = mime_type.replace('/', '_').split(';')[0] # Type générique basé sur MIME
            else: file_type_by_magic = "unknown_mime_none" # magic.from_file a retourné None
            ...
            if file_type_by_magic in text_like_types:
                try:
                    ...
                    if file_type_by_magic == "python_script": return "python_script" # Priorité à magic pour .py
                ...
                except Exception: return file_type_by_magic # En cas d'erreur de lecture, retourner le type de magic
            return file_type_by_magic # Pour les types non textuels identifiés par magic
        except Exception as e: 
            log_callback(f"[AI_ANALYZER] Avertissement: Erreur python-magic pour {os.path.basename(file_path)}: {e}")
    ...
    try: 
        ...
        if not content_sample_bytes: return "empty_file" # Redondant avec la vérif de taille, mais sûr

        if b'\0' in content_sample_bytes[:256]: # Heuristique plus forte pour binaire (octet nul au début)
        ...
        try:
            ...
            non_printable = sum(1 for char_code in content_sample_bytes if not (32 <= char_code <= 126 or char_code in [9,10,13]))
            if non_printable / len(content_sample_bytes) > 0.2: # Si plus de 20% de non-imprimables
        ...
        except UnicodeDecodeError: return "unknown_binary_read_error" # Clairement pas du texte UTF-8
...
def calculate_shannon_entropy(file_path, sample_size=10240, log_callback=_default_log_analyzer):
    if not os.path.exists(file_path): return 0.0 # Fichier non trouvé
    file_s = os.path.getsize(file_path)
    if file_s == 0: return 0.0 # Fichier vide
    
    actual_sample_size = min(file_s, sample_size) # Ne pas lire plus que la taille du fichier
    try:
        with open(file_path, 'rb') as f: data = f.read(actual_sample_size)
        if not data: return 0.0
        
        byte_counts = Counter(data)
        total_bytes = len(data) # Utiliser len(data) qui est actual_sample_size ou moins
        entropy = 0.0
...
if __name__ == '__main__':
    ...
    test_dir = "temp_analyzer_test_files"; os.makedirs(test_dir, exist_ok=True)
    file1_path = os.path.join(test_dir, "test1.txt"); file2_path = os.path.join(test_dir, "test2.py"); file3_path = os.path.join(test_dir, "test3_random.bin")
    with open(file1_path, "w") as f: f.write("this is a simple english text file with some repetition repetition."*10)
    with open(file2_path, "w") as f: f.write("import os\ndef hello():\n  print('world')\n# comment"*10)
    with open(file3_path, "wb") as f: f.write(os.urandom(2048))
    print(f"\n--- Test de get_file_features ---")
    for f_path_test in [file1_path, file2_path, file3_path, __file__]: # Teste lui-même aussi
        if os.path.exists(f_path_test): print(f"  - Fichier: {os.path.basename(f_path_test)}, Features: {get_file_features(f_path_test)}")
