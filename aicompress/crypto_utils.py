...
try:
    ...
    _default_log_crypto(
        "AVERTISSEMENT (crypto_utils.py): 'cryptography' non trouvée. Chiffrement/Déchiffrement désactivé."
    )
...
def encrypt_data(
    data: bytes, password: str, log_callback=_default_log_crypto
) -> tuple | None:
    """
    ...
    if not CRYPTOGRAPHY_AVAILABLE:
        log_callback(
            "[CRYPTO_UTILS] Bibliothèque Cryptography non disponible. Chiffrement impossible."
        )
    ...
    if key is None:  # Ne devrait pas arriver si password est non vide
        log_callback(
            "[CRYPTO_UTILS] Échec de la dérivation de la clé (pas de mot de passe?)."
        )
        return None
    ...
    try:
        ...
        tag = (
            encryptor.tag
        )  # Obtenir le tag GCM (très important pour l'intégrité et l'authenticité)
...
def decrypt_data(
    encrypted_data_payload: bytes,
    password: str,
    salt: bytes,
    iv: bytes,
    tag: bytes,
    log_callback=_default_log_crypto,
) -> bytes | None:
    """
    ...
    if not CRYPTOGRAPHY_AVAILABLE:
        log_callback(
            "[CRYPTO_UTILS] Bibliothèque Cryptography non disponible. Déchiffrement impossible."
        )
        return None
    if (
        not password
    ):  # Si les données étaient chiffrées, un mot de passe est absolument nécessaire
    ...
    if key is None:
        log_callback(
            "[CRYPTO_UTILS] Échec de la dérivation de la clé pour le déchiffrement."
        )
        return None
    ...
    try:
        ...
        log_callback(
            f"[CRYPTO_UTILS] ÉCHEC DU DÉCHIFFREMENT (AES-GCM): {e}. Mot de passe incorrect ou données corrompues/altérées."
        )
        return None
