...
get_file_features_orch = lambda fp, lc: {
    "type": "analyzer_unavailable_in_orch",
    "error": True,
}
try:
    from .ai_analyzer import (
        get_file_features as _real_gff_orch,
    ...
    _default_log_orchestrator(
        f"Dépendance ai_analyzer: {'Prêt' if AI_ANALYZER_READY_ORCH else 'Non Prêt'}"
    )
except ImportError:
...
try:
    from .classic_compressors import (
        ...
        METHOD_BROTLI as _M_BR_cl,  # Constantes de base
    ...
    _default_log_orchestrator(
        f"Dépendance classic_compressors: {'Prêt' if CLASSIC_COMPRESSORS_READY_ORCH else 'Non Prêt'}"
    )
    if CLASSIC_COMPRESSORS_READY_ORCH:
        _default_log_orchestrator(
            f"  ZSTD: {'OK' if ZSTD_READY_ORCH else 'X'}, Brotli: {'OK' if BROTLI_READY_ORCH else 'X'}"
        )
except ImportError:
...
ORCHESTRATOR_MODEL_PATH_ORCH = os.path.join(
    _MODULE_DIR_ORCH, "compression_orchestrator_model.joblib"
)
ORCHESTRATOR_LABEL_ENCODER_PATH_ORCH = os.path.join(
    _MODULE_DIR_ORCH, "orchestrator_label_encoder.joblib"
)
...
def load_orchestrator_model(log_callback=_default_log_orchestrator):
    ...
    if ORCHESTRATOR_IS_READY:
        return True
    if not (
        os.path.exists(ORCHESTRATOR_MODEL_PATH_ORCH)
        and os.path.exists(ORCHESTRATOR_LABEL_ENCODER_PATH_ORCH)
    ):
        log_callback(f"ERREUR: Modèle Orchestrateur ou Label Encoder non trouvé.")
        ORCHESTRATOR_IS_READY = False
        return False
    try:
        import joblib
        ...
        log_callback(
            f"Chargement du Label Encoder: {ORCHESTRATOR_LABEL_ENCODER_PATH_ORCH}"
        )
        _orchestrator_label_encoder_internal = joblib.load(
            ORCHESTRATOR_LABEL_ENCODER_PATH_ORCH
        )
        ...
        log_callback(
            f"ERREUR de dépendance lors du chargement du modèle: {e_mnfe} (pandas ou sklearn manquant?)"
        )
        ORCHESTRATOR_IS_READY = False
        return False
...
if _initial_orchestrator_load_status_unused_ := load_orchestrator_model(
    _default_log_orchestrator
):
    pass
...
def get_compression_settings(
    file_path: str,
    analysis_result_str_ignored: str,
    log_callback=_default_log_orchestrator,
) -> tuple:
    log_callback(
        f"Début get_compression_settings pour: '{os.path.basename(file_path)}'"
    )

    if not ORCHESTRATOR_IS_READY:
        log_callback(
            "ERREUR CRITIQUE: Modèle orchestrateur non chargé. Fallback: DEFLATE L6."
        )
        return METHOD_DEFLATE, {"level": 6}

    if not AI_ANALYZER_READY_ORCH:
        log_callback(
            "AI Analyzer non disponible (via orchestrator). Fallback: DEFLATE L6."
        )
        return METHOD_DEFLATE, {"level": 6}
    ...
    if features.get("error"):
        log_callback(
            f"Erreur extraction features pour {os.path.basename(file_path)}. Fallback: DEFLATE L6."
        )
        return METHOD_DEFLATE, {"level": 6}
    ...
    try:
            ...
            [
                {
                    ...
                    "quick_comp_ratio": features.get(
                        "quick_comp_ratio", 1.0
                    ),  # S'assurer que quick_comp_ratio a un fallback
                }
            ]
        )
    except ImportError:
        ...
        log_callback(
            f"Erreur création DataFrame pour prédiction: {e_df}. Fallback DEFLATE L6."
        )
        return METHOD_DEFLATE, {"level": 6}

    try:
        predicted_method_encoded = _orchestrator_pipeline_internal.predict(input_df)
        predicted_method_name = _orchestrator_label_encoder_internal.inverse_transform(
            predicted_method_encoded
        )[0]
        log_callback(
            f"Méthode prédite par IA pour '{os.path.basename(file_path)}': {predicted_method_name}"
        )
    except Exception as e_predict:
        log_callback(
            f"Erreur lors de la prédiction par l'orchestrateur: {e_predict}. Fallback DEFLATE L6."
        )
        return METHOD_DEFLATE, {"level": 6}
    ...
    if predicted_method_name == METHOD_STORED:
        ...
        try:
            level = int(predicted_method_name.split("_L")[1])
            return METHOD_DEFLATE, {"level": level}
        except:
            log_callback(f"Erreur parsing DEFLATE: {predicted_method_name}")
            return METHOD_DEFLATE, {"level": 6}
    elif predicted_method_name.startswith(METHOD_BZIP2 + "_L"):
        try:
            level = int(predicted_method_name.split("_L")[1])
            return METHOD_BZIP2, {"level": level}
        except:
            log_callback(f"Erreur parsing BZIP2: {predicted_method_name}")
            return METHOD_BZIP2, {"level": 9}
    elif predicted_method_name.startswith(METHOD_LZMA + "_P"):
        try:
            preset = int(predicted_method_name.split("_P")[1])
            return METHOD_LZMA, {"preset": preset}
        except:
            log_callback(f"Erreur parsing LZMA: {predicted_method_name}")
            return METHOD_LZMA, {"preset": 6}
        ...
        try:
            level = int(predicted_method_name.split("_L")[1])
            return METHOD_ZSTD, {"level": level}
        except:
            log_callback(f"Erreur parsing ZSTD: {predicted_method_name}")
            return METHOD_ZSTD, {"level": 3}
        ...
        try:
            ...
            log_callback(
                f"Erreur parsing qualité Brotli: {predicted_method_name}. Fallback BROTLI Q6."
            )
    ...
    #   ... (logique AE supprimée) ...
        ...
        log_callback(
            f"Méthode prédite '{predicted_method_name}' non gérée ou AE (supprimé). Fallback DEFLATE L6."
        )
        return METHOD_DEFLATE, {"level": 6}
